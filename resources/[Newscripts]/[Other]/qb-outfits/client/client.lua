...
RegisterNetEvent('qb-outfits:client:promptSaveOutfit', function()
        ...
        inputs = {
            {
                ...
                isRequired = true,
                name = 'outfitNumber',
                text = 'Enter Outfit Number (1 - 25)'
            }
        }
    })

    if dialog then
        local outfitNumber = tonumber(dialog.outfitNumber)
        if outfitNumber and outfitNumber > 0 and outfitNumber <= 25 then
            TriggerServerEvent('qb-outfits:server:saveOutfit', outfitNumber)
        else
            QBCore.Functions.Notify("Wrong outfit number [1 - 25]", "error")
        end
    end
end)

function OpenOutfitsMenu(force)
    local args = {}
    if force then
        table.insert(args, "--force")
    end
    TriggerServerEvent("OriDN:outfitsCommand", args)
end


RegisterNetEvent('OriDN:outfitsCommand:client', function()
    OpenOutfitsMenu(force)
end)

RegisterNetEvent('qb-outfits:client:opennewmenu')
AddEventHandler('qb-outfits:client:opennewmenu', function()
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            icon = 'fas fa-save', 
			params = {
                event = 'qb-outfits:client:promptSaveOutfit'
            }
        },
        {
			id = 3,
            ...
            icon = 'fas fa-tshirt', 
			params = {
                event = 'OriDN:outfitsCommand:client'
            }
        },
        {
			id = 4,
