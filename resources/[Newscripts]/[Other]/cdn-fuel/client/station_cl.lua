...
if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    local function UpdateStationInfo(info)
        ...
        -- Return the current state for the advanced UI
        return {
    ...
    RegisterNUICallback('toggleEmergencyShutoff', function(data, cb)
    ...
    RegisterNUICallback('changeFuelPrice', function(data, cb)
        local location = data.location
        local fuelprice = tonumber(data.price)
        
        if fuelprice < Config.MinimumFuelPrice then 
            cb({status = 'error', message = Lang:t("station_price_too_low")})
            return
        end
        
        if fuelprice > Config.MaxFuelPrice then
            cb({status = 'error', message = Lang:t("station_price_too_high")})
            return
        end
        
        TriggerServerEvent("cdn-fuel:station:server:updatefuelprice", fuelprice, location)
        cb({status = 'success'})
    end)

    RegisterNUICallback('purchaseReserves', function(data, cb)
        local location = data.location
    ...
    RegisterNUICallback('changeStationName', function(data, cb)
        local location = data.location
        local newName = data.name
        
        if string.len(newName) > Config.NameChangeMaxChar then 
            cb({status = 'error', message = Lang:t("station_name_too_long")})
            return
        end
        
        if string.len(newName) < Config.NameChangeMinChar then 
            cb({status = 'error', message = Lang:t("station_name_too_short")})
            return
        end
        
        if Config.ProfanityList[newName] then 
            cb({status = 'error', message = Lang:t("station_name_invalid")})
            return
        end
        
        TriggerServerEvent("cdn-fuel:station:server:updatelocationname", newName, location)
        cb({status = 'success'})
    end)
    ...
    RegisterNUICallback('withdrawFunds', function(data, cb)
        ...
        FetchStationInfo('balance')
        Wait(100)
        
        if amount < 1 then 
            cb({status = 'error', message = Lang:t("station_withdraw_too_little")})
            return
        end
        
        if amount > StationBalance then 
            cb({status = 'error', message = Lang:t("station_withdraw_too_much")})
            return
        end
    ...
    RegisterNUICallback('depositFunds', function(data, cb)
        ...
        if amount < 1 then 
            cb({status = 'error', message = Lang:t("station_deposit_too_little")})
            return
        end
        
        if amount > bankmoney then 
            cb({status = 'error', message = Lang:t("not_enough_money_in_bank")})
            return
        end
    ...
    -- Function to open the advanced UI
    function OpenAdvancedStationUI()
        ...
        local stationData = {
            ...
            name = Config.GasStations[CurrentLocation].label,
            owned = isOwned,
            isOwner = isOwner,
            shutoff = shutoffStatus,
            reserves = ReserveLevels or 0,
            maxReserves = Config.MaxFuelReserves,
            fuelPrice = StationFuelPrice or Config.CostMultiplier,
            balance = StationBalance or 0,
            cost = Config.GasStations[CurrentLocation].cost,
            taxRate = Config.GlobalTax,
            sellPercentage = Config.GasStationSellPercentage,
            minFuelPrice = Config.MinimumFuelPrice,
            maxFuelPrice = Config.MaxFuelPrice,
            nameChangeEnabled = Config.GasStationNameChanges,
            nameChangeMinChar = Config.NameChangeMinChar,
            nameChangeMaxChar = Config.NameChangeMaxChar,
            reservePrice = Config.FuelReservesPrice
        }
        
        local playerData = {
            bankBalance = playerMoney.bank,
            cashBalance = playerMoney.cash
        }
    ...
    -- Override the existing 'cdn-fuel:stations:openmenu' event to use our advanced UI
    RegisterNetEvent('cdn-fuel:stations:openmenu', function()
        if Config.UseAdvancedStationUI then
            OpenAdvancedStationUI()
        else
            -- Call the original function to use qb-menu
            TriggerEvent('cdn-fuel:stations:original:openmenu')
        end
    end)

    -- Create a copy of the original event to use when advanced UI is disabled
    RegisterNetEvent('cdn-fuel:stations:original:openmenu', function()
    ...
    -- Add function to close UI when resource stops
