...
AddEventHandler("qb-casino:server:GoldSell", function()
    ...
    if xItem ~= nil then
        ...
        TriggerClientEvent("nchub::casinoChipMenu", src)
		quantity = 0
    else
...
RegisterNetEvent("nchub::server:purchaseMembership", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MembershipCheck = Player.Functions.GetItemByName('casino_members')
    if MembershipCheck ~= nil then
        TriggerClientEvent('nchub::casinoMembershipMenu', src)
...
RegisterNetEvent("nchub::server:purchaseVIPMembership", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nchub::server:HasCasinoMembership', function(source, cb)
...
QBCore.Functions.CreateCallback('nchub::server:HasVIPMembership', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
-- RegisterNetEvent("nchub::server:validateMembership")
-- AddEventHandler("nchub::server:validateMembership", function(args)
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
--     local args = tonumber(args)
-- 	if args == 1 then 
--         if Player.Functions.GetItemByName("casino_members") then
--             Player.Functions.RemoveItem("casino_members", 1)
--             local info = {
...
--             TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["casino_members_validated"], "add", 1) 
